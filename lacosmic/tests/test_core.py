...
class TestRemoveCosmics:
    """
    Tests for remove_cosmics.
    """

    def test_remove_cosmics(self):
        """
        Test basic remove_cosmics.
        """
        crclean_img, crmask_img = remove_cosmics(CR_IMG, 1, 40, 50,
                                                 error=ERROR)
    ...
    def test_background_scalar(self):
        """
        Test remove_cosmics with a background scalar.
        """
        crclean_img, crmask_img = remove_cosmics(CR_IMG, 1, 40, 50,
                                                 error=ERROR, background=10)
        assert_allclose(crclean_img, IMG, atol=2.)
        assert_array_equal(crmask_img, MASK_REF)
    ...
    def test_background_image(self):
        """
        Test remove_cosmics with a 2D background image.
        """
    ...
    def test_mask_image(self):
        """
        Test remove_cosmics with an input mask image.
        """
        mask = MASK_REF.copy()
    ...
    def test_large_cosmics(self):
        """
        Test remove_cosmics cleaning with large cosmic rays.
        """
        test_img = np.ones((7, 7))
        test_img[1:6, 1:6] = 100.
        mask_ref2 = np.zeros((7, 7), dtype=bool)
        mask_ref2[1:6, 1:6] = True
        _, crmask_img = remove_cosmics(test_img, 3, 2, 2, effective_gain=1,
                                       readnoise=0)
        assert_array_equal(crmask_img, mask_ref2)
    ...
    def test_error_image_size(self):
        ...
        with pytest.raises(ValueError, match=match):
            remove_cosmics(CR_IMG, 3, 2, 2, effective_gain=1, readnoise=0,
                           error=error)

    def test_error_inputs(self):
        match = 'effective_gain and readnoise must be input if error is not'
        with pytest.raises(ValueError, match=match):
            remove_cosmics(CR_IMG, 3, 2, 2, effective_gain=1, error=None)

        with pytest.raises(ValueError, match=match):
            remove_cosmics(CR_IMG, 3, 2, 2, readnoise=0, error=None)
